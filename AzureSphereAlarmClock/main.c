...
#include "applibs_versions.h" //Defines the API struct versions to use for applibs API
#include <applibs/log.h>
...
#include "epoll_timerfd_utilities.c"
...
//Init and Termination Handlers
static int InitPeripheralsAndHandlers(void);
static void ClosePeripheralsAndHandlers(void);

//Button A Attributes
static int buttonAGPIO_ID = 12;
static int buttonAfd = -1;
...
/// <summary>
/// Pressing button A will Send a 'Button Pressed' event to Azure IoT Central
/// </summary>
static void PollButtonA(void)
{
	if (IsButtonPressed(buttonAfd, &buttonAState))
	{
		Log_Debug("Would send message--> Instance %d", count);
		count++;
		//TODO: would send message to IoTHub
		SendTelemetry("ButtonPress", "True");
	}
}

/// <summary>
/// Triggered on the Button Polling Timer, periodically polls whether button is pressed
/// </summary>
static void ButtonPollEventHandler(EventData *eventData)
{
...
//IoTHub Functions
void SendDataToIoTHub()
{
	//Calls SendTelemetry to actually send the data
	Log_Debug("Would Send Data to IoTHub\n");
}
...
/// </summary>
static void AzureMssgTimerEventHandler(EventData *eventData)
{
	Log_Debug("Updating Azure Client Through Timer\n");
...
static EventData buttonPollEventData = { .eventHandler = &ButtonPollEventHandler };
static EventData azureEventData = { .eventHandler = &AzureMssgTimerEventHandler };
...
static void TwinCallback(DEVICE_TWIN_UPDATE_STATE updateState, const unsigned char *payload,
	size_t payloadSize, void *userContextCallback)
{
	Log_Debug("Message Received from IotHub- Figure out how to handle that for your app");
...
static int InitPeripheralsAndHandlers(void)
{
	...
	//Open Button A as GPIO Input
	Log_Debug("Opening Button A as input\n");
	buttonAfd = GPIO_OpenAsInput(buttonAGPIO_ID);
	if (buttonAfd < 0)
	{
		Log_Debug("ERROR: Could not open Button A as input: %s (%d).\n", strerror(errno), errno);
		return -1;
	}
...
//	}
	...
	CloseFdAndPrintError(buttonAfd, "ButtonA");
...
int old_main(int argc, char *argv[])
{
	...
	{
		Log_Debug(
			"Error opening Button A GPIO: %s (%d). Check that app_manifest.json includes the GPIO used.\n",
			strerror(errno), errno);
		return -1;
	}
	...
	while (true)
	{
		...
		GPIO_GetValue(buttonAfd, &result);
		if (result == GPIO_Value_Low)
		{
